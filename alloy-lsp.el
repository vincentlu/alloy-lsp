...
;;
;; The server JAR is downloaded automatically on first use.
;; To use a custom JAR, set `alloy-lsp-server-command':
...
(defvar alloy-lsp--jar-url
  "https://github.com/AlloyTools/org.alloytools.alloy/releases/download/v6.2.0/org.alloytools.alloy.dist.jar"
  "URL to download the Alloy language server JAR.")

(defvar alloy-lsp--jar-name "org.alloytools.alloy.dist.jar"
  "Filename of the Alloy language server JAR.")

(defun alloy-lsp--jar-path ()
  "Return the path to the Alloy JAR, downloading it if necessary."
  (let ((path (expand-file-name alloy-lsp--jar-name alloy-lsp--directory)))
    (unless (file-exists-p path)
      (message "Alloy LSP: downloading server JAR (~20MB)...")
      (redisplay t)
      (let ((inhibit-message t))
        (url-copy-file alloy-lsp--jar-url path))
      (message "Alloy LSP: server JAR downloaded to %s" path)
      (warn "alloy-lsp: The official v6.2.0 JAR has a broken evaluator. \
Solutions will not be explorable until a new upstream release is available."))
    path))

(defcustom alloy-lsp-server-command nil
  "Command list to start the Alloy language server in stdio mode.
If nil, defaults to running the JAR next to `alloy-lsp.el'
\(downloading it from the official Alloy release if needed)."
...
(defcustom alloy-lsp-auto-open-instance t
  "If non-nil, automatically open the instance when a run finds one.
When a RunResult arrives with a link, send `OpenModel' immediately
instead of requiring a manual click."
  :type 'boolean)

(defcustom alloy-lsp-output-window-side 'bottom
...
(defun alloy-lsp-clear-output ()
  "Clear the Alloy output buffer."
  (interactive)
  (let ((inhibit-read-only t))
    (erase-buffer)
    (setq alloy-lsp--last-replace-node nil)))

(define-key alloy-lsp-output-mode-map (kbd "c") #'alloy-lsp-clear-output)
(define-key alloy-lsp-output-mode-map (kbd "s") #'alloy-lsp-stop-execution)
;; evil-mode remaps all editing keys to `ignore' in special-mode derivatives;
;; bind in evil's normal state so keys aren't swallowed.
(with-eval-after-load 'evil
  (evil-define-key 'normal alloy-lsp-output-mode-map
    (kbd "c") #'alloy-lsp-clear-output
    (kbd "s") #'alloy-lsp-stop-execution))
...
(defun alloy-lsp--on-show-execution-output (workspace params)
  ...
  (let* ((msg (or (gethash "message" params) ""))
    ...
    ;; Auto-open instance when a result arrives with a link
    (when (and alloy-lsp-auto-open-instance link (= msgtype 2))
      (alloy-lsp-open-model link))))
...
(defvar alloy-lsp--client
  (make-lsp-client
   :new-connection (lsp-stdio-connection
                    (lambda ()
                      (or alloy-lsp-server-command
                          (list "java" "-jar" (alloy-lsp--jar-path) "ls"))))
   :major-modes '(alloy-mode alloy-ts-mode markdown-mode)
   :server-id 'alloy-lsp
